// AICUT 블로그 이미지 5장 일괄 생성 (교육 주제)
const { makeImage } = require('./skills/image_gen');

(async () => {
  const images = [
    {
      // 대표 이미지 (700x700)
      theme: 'dark_green',
      badge: '📚 교육 콘텐츠',
      main: '온라인 강사라면\n<em>숏폼 마케팅</em>을\n시작해야 하는 이유',
      sub: '방학 특강 시즌, 지금이 기회입니다',
      cta: 'AICUT 무료상담 →',
      width: 700, height: 700,
      out: 'aicut_blog_edu_01_main.png'
    },
    {
      // 본문 이미지 1 (800x450)
      theme: 'dark_green',
      badge: '📈 시장 트렌드',
      main: '온라인 강의 시장 10조 시대\n<em>마케팅</em>이 경쟁력이다',
      sub: '강의 퀄리티만큼 중요한 홍보 전략',
      cta: 'AICUT과 준비하세요',
      width: 800, height: 450,
      out: 'aicut_blog_edu_02_market.png'
    },
    {
      // 본문 이미지 2 (800x450)
      theme: 'light_cyan',
      badge: '🎬 숏폼 마케팅',
      main: '숏폼 영상이 강의 홍보에\n효과적인 <em>3가지 이유</em>',
      sub: '릴스·쇼츠·틱톡 알고리즘 활용법',
      cta: 'AICUT 편집 맡기기',
      width: 800, height: 450,
      out: 'aicut_blog_edu_03_shortform.png'
    },
    {
      // 본문 이미지 3 (800x450)
      theme: 'dark_purple',
      badge: '🔥 시즌 전략',
      main: '방학 특강 시즌\n<em>지금 시작</em>해야 하는 이유',
      sub: '6월 말~7월 초가 골든타임',
      cta: '지금 준비하세요',
      width: 800, height: 450,
      out: 'aicut_blog_edu_04_season.png'
    },
    {
      // 본문 이미지 4 (800x450) - CTA
      theme: 'light_pink',
      badge: '🎯 영상 편집 아웃소싱',
      main: '강의 영상 마케팅\n<em>AICUT</em>과 함께하세요',
      sub: '월 정기 납품으로 안정적인 콘텐츠 발행',
      cta: '무료 상담 → master@aicut.co.kr',
      width: 800, height: 450,
      out: 'aicut_blog_edu_05_cta.png'
    }
  ];

  console.log('=== AICUT 블로그 이미지 생성 시작 ===');
  for (let i = 0; i < images.length; i++) {
    const img = images[i];
    console.log(`[${i + 1}/5] 생성 중: ${img.out} (${img.width}x${img.height})...`);
    const result = await makeImage(img);
    console.log(`  ✅ ${result.file} (${result.sizeKB}KB)`);
  }
  console.log('=== 전체 생성 완료 ===');
})();
