...
(async () => {
  ...
  // "작성중인 글이 있습니다" 팝업 → 정확히 "아니오" 찾기
  console.log('\n[팝업] "아니오" 버튼 찾기...');
  
  // 팝업 스크린샷
  await page.screenshot({ path: path.join(WORKSPACE, 'debug_popup.png'), fullPage: false });
  console.log('  📸 스크린샷 저장: debug_popup.png');
  ...
  const noBtn = await page.evaluate(() => {
    ...
    for (const el of all) {
      ...
      if (t === '아니오') {
        el.click();
        return 'found EXACT: 아니오 | ' + el.tagName + ' | ' + (el.className || '').substring(0, 40);
      }
    }
    // 2. "아니오" 포함
    for (const el of all) {
      ...
      if (t.includes('아니오')) {
        el.click();
        return 'found INCLUDES: ' + t.substring(0, 15) + ' | ' + el.tagName;
      }
    }
    // 3. 모든 span/button 텍스트 로깅
    const texts = [];
    const btns = document.querySelectorAll('button, a, span, div');
    btns.forEach(b => {
      const t = (b.innerText || '').trim();
      if (t.length > 0 && t.length < 10) texts.push(t);
    });
    return 'NOT FOUND — available texts: ' + [...new Set(texts)].join(', ');
  });
  console.log('  결과:', noBtn);
  await sleep(2000);
  ...
  console.log('\n=== 저장 직전 ===');
  console.log('제목:', beforeSave.title ? '✅' : '❌');
  console.log('해시태그:', beforeSave.tagCount + '개');
  
  // 5. 저장
  if (beforeSave.title && beforeSave.tagCount >= 20) {
    console.log('\n[4] 저장...');
    await page.evaluate(() => {
    ...
    // 확인: 같은 에디터에서 제목 다시 읽기
    const after = await page.evaluate(() => {
      const r = {};
      try { r.title = SmartEditor._editors['blogpc001'].getDocumentTitle() || ''; } catch(e) { r.title = ''; }
      return r;
    });
    console.log('  저장 후 제목:', after.title ? '✅ 유지' : '❌ 사라짐');
    ...
    const result = await checkPage.evaluate(() => {
      ...
      try {
        ...
        r.types = (d.document?.components || []).slice(0,3).map(c => c['@ctype'] || c.type);
      } catch(e) {}
      const inputs = document.querySelectorAll('input');
      for (const inp of inputs) {
        if ((inp.placeholder || '').includes('글감')) { r.tagCount = inp.value.split('#').filter(t => t.trim().length > 0).length; break; }
    ...
    console.log('\n=== 최종 저장 확인 ===');
    console.log('제목:', result.title ? '✅ ' + result.title.substring(0, 40) + '...' : '❌ 빈 제목');
    console.log('컴포넌트:', result.compCount + '개');
    console.log('타입:', result.types?.join(', '));
