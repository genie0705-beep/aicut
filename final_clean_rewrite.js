...
// 레퍼런스 스타일 본문 (FP 포스트 스타일: 짧은 문단, 이모지, 수치, 사례)
const posts = [
  {
...
(async () => {
  ...
  // dialog 자동 수락
  ctx.on('page', page => { page.on('dialog', d => { d.accept().catch(()=>{}); }); });

  for (let pi = 0; pi < posts.length; pi++) {
    const post = posts[pi];
    console.log(`\n━━━ [${pi+1}/2] ${post.title.substring(0, 30)}... ━━━`);
    ...
    for (let i = 0; i < post.lines.length; i++) {
      if (post.lines[i] === '') { await page.keyboard.press('Enter'); continue; }
      await page.keyboard.type(post.lines[i], { delay: 3 });
    ...
    for (let imgIdx = 0; imgIdx < 6; imgIdx++) {
      ...
      try {
        ...
        else { console.log(`      ⚠️ ${imgFile} 실패`); break; }
      } catch(e) { console.log(`      ⚠️ ${imgFile} 오류`); break; }
    }
