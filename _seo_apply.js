...
(async () => {
  ...
  // ===== SEO 보강 본문 =====
  // 추가 키워드: 분양대행사(4~5회), 영상 마케팅(4~5회), 숏폼(3~4회), 하반기(3~4회)
  
  // 1. MAIN 이미지
  console.log('[1] 메인 이미지...');
  await up('aicut_blog_estate_main.png');
  
  // 2. 도입부 — 키워드: 분양대행사(1), 영상마케팅(1), 하반기(1)
  console.log('[2] 도입부...');
  ...
  await br();
  await w('"영상 제작 견적이 천차만별이고 뭐가 맞는지 모르겠고."');
  await br();
  await w('"하반기 준비해야 하는데 시간은 너무 부족하고."');
  await br(); await br();
  await w('이게 많은 분양대행사 관계자분들의 현실입니다.');
  await br();
  await w('분야를 막론하고 영상 마케팅이 선택이 아닌 필수가 된 시대,');
  await br();
  await w('분양대행사도 예외는 아닙니다.');
  await br();
  await w('저 역시 3개월간의 시행착오 끝에 해결책을 찾았습니다.');
  await br();
  ...
  // H2-like: 🔄 섹션 — 키워드: 분양대행사(2), 영상(1)
  await w('🔄 분양대행사가 직접 하려다 부딪힌 3개월');
  await br(); await br();
  ...
  await br();
  await w('화질이 들쭉날쭉했고, 편집에 4~5시간씩 투자하다 보니');
  await br();
  await w('본업인 분양 대행 업무에 지장이 생기기 시작했습니다.');
  await br(); await br();
  await w('3달 차, 결론을 내렸습니다.');
  await br();
  await w('"이거 차라리 전문 영상 편집 업체에 맡기는 게 낫겠다."');
  await br();
  ...
  // H2-like: 💡 섹션 — 키워드: 영상마케팅(2), 분양대행사(3), 하반기(2)
  await w('💡 영상 마케팅, 직접 vs 외주 냉정한 계산');
  await br(); await br();
  await w('분양대행사 입장에서 영상 마케팅을 직접 제작할 때의 비용을 계산해봤습니다.');
  await br();
  await w('전담 인력 1명의 월 인건비는 최소 300만원부터.');
  await br();
  await w('장비 비용과 교육 시간까지 더하면 부담은 더 커집니다.');
  await br(); await br();
  ...
  // H2-like: 📱 섹션 — 키워드: 숏폼(2), 영상마케팅(3)
  await w('📱 분양대행사 영상 마케팅, 채널별 맞춤 전략');
  await br(); await br();
  await w('같은 분양 영상이라도 플랫폼에 따라 숏폼 접근 방식이 달라야 합니다.');
  await br();
  await w('인스타그램 릴스는 감성적인 인테리어 위주의 숏폼 영상이 효과적.');
  await br();
  await w('모델하우스 디테일을 잔잔한 BGM과 함께 보여주면');
  await br();
  await w('예비 청약자들의 저장률이 크게 올라갑니다.');
  await br(); await br();
  await w('유튜브 쇼츠는 정보 전달형 숏폼 콘텐츠에 강합니다.');
  await br();
  await w('"이번 달 청약 일정", "지역별 분양가 비교" 등');
  await br();
  await w('실질적 도움을 주는 영상 마케팅을 30~60초로 압축하면');
  await br();
  await w('구독자 유입에 효과적입니다.');
  await br(); await br();
  await w('틱톡은 트렌디하고 가벼운 접근이 필요합니다.');
  await br();
  await w('분양 현장 비하인드, 관계자의 일상 등');
  await br();
  await w('진정성 있는 숏폼 콘텐츠로');
  await br();
  ...
  // H2-like: ✅ 섹션 — 키워드: 분양대행사(4), 하반기(3)
  await w('✅ 분양대행사 영상 마케팅, 에이컷 도입 후 바뀐 점');
  await br(); await br();
  await w('첫째, 정시 납품이 가능해졌습니다.');
  await br();
  await w('매주 정해진 요일에 맞춰 숏폼 영상이 도착하니');
  await br();
  await w('하반기 마케팅 일정을 체계적으로 관리할 수 있게 되었습니다.');
  await br(); await br();
  await w('둘째, 퀄리티가 확연히 올라갔습니다.');
  await br();
  await w('전문 에디터가 작업한 영상 마케팅 자료는');
  await br();
  await w('조명, 색보정, 자막 디자인 모든 면에서 완성도가 높습니다.');
  await br(); await br();
  await w('셋째, 비용이 오히려 절감됐습니다.');
  await br();
  await w('분양대행사 전담 인력을 고용하는 것보다');
  await br();
  await w('월 50% 이상 비용을 아끼면서 더 많은 콘텐츠를 생산할 수 있었습니다.');
  await br(); await br();
  ...
  // CTA — 키워드: 하반기(4), 영상마케팅(4), 숏폼(3), 분양대행사(5)
  await w('🎯 하반기 영상 마케팅, 지금 준비하세요');
  await br(); await br();
  ...
  await br();
  await w('하반기 마케팅 전략부터 숏폼 영상 제작, 정기 납품까지');
  await br();
