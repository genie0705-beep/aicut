...
(async () => {
  ...
  // 2. HTML 준비
  const html = '<h2 style="text-align:center">무더위, 보험설계사에게 가장 힘든 계절이 돌아왔습니다.</h2><p style="text-align:center">7월 한낮 기온이 33도를 넘나듭니다. 보험설계사라면 누구나 공감할 거예요. 더운 날씨에 만남 잡기도 어렵고, 하반기 준비는 해야 하는데 막막하기만 하죠.</p>';
  
  // 3. 데이터 모델 직접 설정 (blocks)
  await se.evaluate((htmlContent) => {
    const ed = SmartEditor._editors['blogpc001'];
    const data = ed.getDocumentData();
    
    // 간단한 blocks 구성 (paragraph 타입만)
    const texts = htmlContent.replace(/<[^>]+>/g, '\n').split('\n').filter(t => t.trim());
    data.document.blocks = [];
    
    // heading2
    data.document.blocks.push({
      type: 'heading2',
      text: '무더위, 보험설계사에게 가장 힘든 계절이 돌아왔습니다.',
      style: { textAlign: 'center' }
    });
    
    // paragraphs
    const lines = [
      '7월 한낮 기온이 33도를 넘나듭니다.',
      '보험설계사라면 누구나 공감할 거예요.',
      '더운 날씨에 만남 잡기도 어렵고,',
      '하반기 준비는 해야 하는데 막막하기만 하죠.'
    ];
    ...
    data.document.blocks.push({
      type: 'heading2',
      text: '☀️ 무더위, 보험설계사가 마주한 3가지 현실',
      style: { textAlign: 'center' }
    });
    
    const section2 = [
      { type: 'heading3', text: '1. 만남 자체가 어려워진 계절', style: { textAlign: 'center' } },
      { type: 'paragraph', text: '무더위에 지친 고객들은 만남을 꺼립니다. "더우니까 다음에 만나요"라는 말에 속이 타들어가는 경험, 다들 있으시죠?', style: { textAlign: 'center' } },
      { type: 'paragraph', text: '', style: { textAlign: 'center' } },
      { type: 'heading3', text: '2. 열정이 식어가는 업계 분위기', style: { textAlign: 'center' } },
      { type: 'paragraph', text: '여름만 되면 영업력이 떨어지는 건 비단 당신만의 문제가 아닙니다. 대부분의 보험설계사가 같은 고민을 해요. 하반기 목표 달성을 위해선 지금부터 움직여야 합니다.', style: { textAlign: 'center' } },
      { type: 'paragraph', text: '', style: { textAlign: 'center' } },
    ...
    for (const block of section2) data.document.blocks.push(block);
    
    // --- 섹션 3 ---
    data.document.blocks.push({
      type: 'heading2',
      text: '🎯 보험설계사 마케팅, 숏폼이 답이다',
      style: { textAlign: 'center' }
    });
    ...
    const section3 = [
      ...
      { type: 'paragraph', text: '진정성 있는 일상 콘텐츠로 고객과의 친밀감을 쌓으세요. 꾸준한 소통이 신뢰를 줍니다.', style: { textAlign: 'center' } },
      { type: 'paragraph', text: '', style: { textAlign: 'center' } },
    ];
    for (const block of section3) data.document.blocks.push(block);
    ...
    for (const block of section4) data.document.blocks.push(block);
    ...
    const section5 = [
      { type: 'paragraph', text: '촬영은 할 수 있어도 편집은 어렵죠. 그래서 영상편집외주가 필요합니다. 에이컷이 도와드립니다.', style: { textAlign: 'center' } },
      { type: 'paragraph', text: '', style: { textAlign: 'center' } },
    ];
    for (const block of section5) data.document.blocks.push(block);
    ...
    for (const block of cta) data.document.blocks.push(block);
    ...
    data.document.blocks.push({
      type: 'paragraph',
      text: '#보험설계사 #무더위 #하반기마케팅 #숏폼마케팅 #영상편집외주 #보험마케팅 #FP마케팅 #릴스마케팅 #인스타릴스 #유튜브쇼츠 #틱톡마케팅 #숏폼콘텐츠 #영상마케팅 #에이컷 #영상편집 #숏폼편집 #보험영업 #보험FP #하반기준비 #영상제작 #마케팅전략 #SNS마케팅 #콘텐츠마케팅 #보험상담 #재무설계 #FP브랜딩 #보험숏폼 #여름마케팅 #7월마케팅 #영상외주',
      style: { textAlign: 'center' }
    });
    ...
    // Canvas 직접 업데이트 (렌더링 트리거)
    const canvas = document.querySelector('.se-canvas');
    if (canvas) {
      // 모든 텍스트를 한 번에 HTML로 변환
      let html = '';
      for (const block of data.document.blocks) {
        const tag = block.type === 'heading2' ? 'h2' : block.type === 'heading3' ? 'h3' : 'p';
        const text = block.text;
        const align = block.style && block.style.textAlign ? block.style.textAlign : 'center';
        if (text) {
          html += '<' + tag + ' style=\"text-align:' + align + '\">' + text + '</' + tag + '>';
        } else {
  ...
  const v = await se.evaluate(() => {
    ...
    return {
      ...
      blockCount: data.document ? data.document.blocks.length : 0,
  ...
  if (v.textLen > 500) {
    console.log('✅ 본문 복구 성공! (' + v.textLen + '자)');
  } else {
    console.log('⚠️ ' + v.textLen + '자 — 불완전');
  }
