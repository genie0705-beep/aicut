...
(async () => {
  ...
  for (let i = 0; i < CARDS.length; i++) {
    ...
    try {
      ...
      // 캡션 입력
      const captionArea = await p.$('[aria-label*="캡션"], [aria-label*="Caption"], textarea, [contenteditable="true"][role="textbox"]');
      if (captionArea) {
      ...
      const shared = await p.evaluate(() => {
        const btn = Array.from(document.querySelectorAll('button'))
          .find(b => b.innerText?.trim() === '공유하기' && !b.disabled);
        if (btn) { btn.click(); return true; }
        return false;
      });
