...
function h3(text) { return '<h3 style="text-align: center;">' + text + '</h3>'; }
...
function buildBody() {
  var lines = [
    ...
    p('#교육크리에이터 #강사 #온라인클래스 #전문강사 #교육플랫폼'),
  ];
...
(async () => {
  ...
  for (var i = 0; i < pages.length; i++) {
    if (pages[i].url().indexOf('aicut') >= 0 && pages[i].url().indexOf('Write') >= 0) {
      page = pages[i]; break;
    }
  }
  if (!page) {
    console.log('❌ 블로그 작성 페이지를 찾을 수 없습니다.');
    console.log('   네이버 블로그 글쓰기 페이지를 먼저 열어주세요.');
    await b.close();
    return;
  }
  ...
  for (var ii = 0; ii < imgFiles.length; ii++) {
    console.log('   이미지 ' + (ii + 1) + '/' + imgFiles.length + ' 업로드 중...');

    // 사진 버튼 찾기 (좌측 상단)
    var photoBtn = null;
    try {
      photoBtn = await ef.evaluate(function() {
        // 사진 버튼 셀렉터: toolbar 내 이미지/사진 관련 버튼
        var btns = document.querySelectorAll('button');
        for (var i = 0; i < btns.length; i++) {
          var t = (btns[i].innerText || '').trim();
          var html = (btns[i].innerHTML || '');
          // '사진' 텍스트 또는 SVG 아이콘 포함 버튼
          if (t === '사진' || html.indexOf('사진') >= 0 || html.indexOf('photo') >= 0 || t.indexOf('사진') >= 0) {
            return true;
          }
          // SVG 이미지 아이콘 체크
          if (html.indexOf('M19') >= 0 && html.indexOf('viewBox') >= 0 && html.indexOf('image') >= 0) {
            return true;
          }
        }
        return false;
      ...
      console.log('   사진 버튼 찾기 실패, 좌표 기반 클릭 시도...');
      photoBtn = false;
    }

    // filechooser 대기 후 사진 버튼 클릭
    var fileChooserPromise = page.waitForEvent('filechooser', { timeout: 10000 }).catch(function(e) { return null; });
    ...
    if (photoBtn) {
      ...
      // mainFrame 영역에서 좌측 상단 툴바 버튼 클릭 시도
      await mf.evaluate(function() {
        var btns = document.querySelectorAll('button');
        for (var i = 0; i < btns.length; i++) {
          var t = (btns[i].innerText || '').trim();
          if (t === '사진' || t.indexOf('사진') >= 0 || t === 'Picture') {
            btns[i].click();
            return;
          }
        }
    ...
    if (fc) {
      ...
      console.log('   ✅ 파일 선택됨: ' + path.basename(imgFiles[ii]));

      // 업로드 완료 대기 (이미지 업로드 시간)
      await sleep(4000);

      // 업로드된 이미지 패널에서 이미지 선택 (본문에 삽입)
      try {
        await ef.evaluate(function() {
          // 이미지 패널 내 이미지 썸네일 클릭
        ...
        console.log('   ⚠️ 이미지 패널 클릭 시도');
        // mainFrame에서 패널 이미지 클릭 시도
        try {
          await mf.evaluate(function() {
            var imgs = document.querySelectorAll('img[src*="files"], img[src*="upload"]');
            for (var i = imgs.length - 1; i >= 0; i--) {
              if (imgs[i].offsetParent !== null) {
                imgs[i].click();
                return;
          ...
          console.log('   ✅ 이미지 삽입됨 (mainFrame)');
          await sleep(2000);
        } catch(e2) {
          console.log('   ⚠️ 이미지 선택 실패, 진행');
        }
      }
    } else {
      console.log('   ⚠️ filechooser 이벤트 타임아웃, 다음 이미지로...');
    }
