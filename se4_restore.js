...
(async () => {
  ...
  const wp = pages.find(p => p.url().includes('Redirect=Write') || p.url().includes('PostWriteForm'));
...
<p style="text-align: center;">#보험설계사 #무더위 #하반기마케팅 #숏폼마케팅 #영상편집외주 #보험마케팅 #FP마케팅 #릴스마케팅 #인스타릴스 #유튜브쇼츠 #틱톡마케팅 #숏폼콘텐츠 #영상마케팅 #에이컷 #영상편집 #숏폼편집 #보험영업 #보험FP #하반기준비 #영상제작 #마케팅전략 #SNS마케팅 #콘텐츠마케팅 #보험상담 #재무설계 #FP브랜딩 #보험숏폼 #여름마케팅 #7월마케팅 #영상외주</p>`;
  ...
  // canvas 초기화 후 직접 innerHTML 설정
  await seFrame.evaluate((h) => {
    const canvas = document.querySelector('.se-canvas');
    if (!canvas) return;
    canvas.innerHTML = h;
  }, html);
  ...
  const verify = await seFrame.evaluate(() => {
    ...
    return {
      title: (typeof SmartEditor !== 'undefined' && SmartEditor._editors && SmartEditor._editors['blogpc001']) ? SmartEditor._editors['blogpc001'].getDocumentTitle() : '',
      textLen: text.length,
      hasBody: text.includes('무더위'),
      hasCTA: text.includes('pf.kakao.com'),
      hasHash: text.includes('#보험설계사'),
      images: (canvas ? canvas.querySelectorAll('img').length : 0)
    };
  });
