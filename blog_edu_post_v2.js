// 📝 에이컷 블로그 - 교육/이러닝 영상편집 포스팅 v2
// =============================================
// SEO 100% 적용 | 모바일 톤앤매너 | 이미지 업로드 포함
// SmartEditor SE4 직접 제어 (iframe 불필요)
// =============================================
...
function buildBody() {
  var lines = [
    ...
    p('— 키워드 하이라이트 포함'),
    br(),
    p('🎯 ' + strong('인트로/아웃트로 제작')),
    p('— 브랜드 아이덴티티 반영'),
    br(),
    p('🎯 ' + strong('챕터 구분 및 화면 전환')),
    p('— 강의 흐름에 맞춰 편집'),
    br(),
    p('🎯 ' + strong('BGM 및 음성 보정')),
    p('— 청취 환경 최적화'),
    br(),
    p('🎯 ' + strong('숏폼 추가 제작')),
    p('— 릴스/쇼츠로 2차 활용'),
...
(async () => {
  ...
  for (var i = 0; i < pages.length; i++) {
    var u = pages[i].url();
    if (u.indexOf('PostWriteForm') >= 0) { page = pages[i]; break; }
  }
  if (!page) {
    console.log('❌ 블로그 작성 페이지(PostWriteForm)를 찾을 수 없습니다.');
    await b.close();
    return;
  }
  ...
  console.log(titleOk ? '   ✅ 제목: ' + title : '   ❌ 제목 입력 실패');
  await sleep(2000);

  // ============================================================
  // 2. 이미지 업로드 (사진 버튼 → filechooser)
  // ============================================================
  console.log('\n2. 이미지 업로드...');
  ...
  for (var ii = 0; ii < imgFiles.length; ii++) {
    console.log('   이미지 ' + (ii + 1) + '/' + imgFiles.length + ' (' + path.basename(imgFiles[ii]) + ')');
    
    // 사진 버튼 찾기 (툴바에서 사진/이미지 관련 버튼)
    var btnClicked = await page.evaluate(function() {
      ...
      for (var i = 0; i < btns.length; i++) {
        ...
        var dataTool = btns[i].getAttribute('data-tool') || '';
        
        // '사진' 텍스트 또는 이미지 업로드 관련 속성 확인
        if (t === '사진' || aria === '사진' || dataTool.indexOf('사진') >= 0 || 
            t.indexOf('사진') >= 0 || aria.indexOf('사진') >= 0 ||
            dataTool.indexOf('image') >= 0 || dataTool.indexOf('picture') >= 0) {
          btns[i].click();
          return { ok: true, by: 'text', text: t, aria: aria };
        }
      }
      // SVG 이미지 아이콘 가진 버튼 찾기
      for (var i = 0; i < btns.length; i++) {
        var svgs = btns[i].querySelectorAll('svg');
        if (svgs.length > 0) {
          var html = btns[i].innerHTML;
          if (html.indexOf('image') >= 0 || html.indexOf('picture') >= 0 || html.indexOf('사진') >= 0) {
            btns[i].click();
            return { ok: true, by: 'svg' };
          }
        }
      }
      return { ok: false, message: '사진 버튼 못 찾음' };
    });

    if (!btnClicked.ok) {
      console.log('   ⚠️ 사진 버튼 못 찾음, SmartEditor toolbar 직접 접근...');
      // SmartEditor 의 사진 추가 기능 직접 호출
      var toolbarClicked = await page.evaluate(function() {
        try {
          var ed = SmartEditor._editors['blogpc001'];
          // SE4 toolbar 접근
          if (ed._toolbar) {
            var toolbar = ed._toolbar;
            // image 관련 버튼 찾기
            if (toolbar._buttons && toolbar._buttons.image) {
              toolbar._buttons.image._onClick();
              return { ok: true, by: 'api' };
            }
            // buttons 객체에서 image 찾기
            for (var key in toolbar) {
              if (key.indexOf('image') >= 0 || key.indexOf('picture') >= 0 || key.indexOf('photo') >= 0) {
                if (typeof toolbar[key]._onClick === 'function') {
                  toolbar[key]._onClick();
                  return { ok: true, by: 'toolbar.' + key };
                }
              }
            }
          }
      ...
      if (!toolbarClicked) {
        console.log('   ⚠️ API 호출도 실패, DOM 클릭 재시도...');
        var domClick = await page.evaluate(function() {
          // 네이버 에디터 - _se2_toolbar 영역에서 사진 버튼 찾기
          var toolbars = document.querySelectorAll('._se2_toolbar, .se2_toolbar, [class*="toolbar"]');
          for (var t = 0; t < toolbars.length; t++) {
            var btns = toolbars[t].querySelectorAll('button, a, span');
            for (var i = 0; i < btns.length; i++) {
              var text = (btns[i].innerText || '').trim();
              var cls = btns[i].className || '';
              if (text === '사진' || cls.indexOf('photo') >= 0 || cls.indexOf('image') >= 0 ||
                  btns[i].getAttribute('title') === '사진' || btns[i].getAttribute('alt') === '사진') {
                btns[i].click();
                return true;
              }
            }
        ...
        if (domClick) {
          console.log('   ✅ toolbar DOM 클릭 성공');
        } else {
          console.log('   ❌ 사진 버튼을 찾을 수 없습니다.');
          continue;
        }
      } else {
        console.log('   ✅ SmartEditor API 호출 성공');
      }
    } else {
      console.log('   ✅ 사진 버튼 클릭 (' + (btnClicked.by || 'dom') + ')');
    }

    await sleep(2000);

    // filechooser 대기
    var fc = await page.waitForEvent('filechooser', { timeout: 15000 }).catch(function(e) { return null; });
    if (fc) {
      await fc.setFiles([imgFiles[ii]]);
      console.log('   ✅ 파일 선택됨');
      await sleep(5000);

      // 업로드된 이미지 썸네일 클릭 (본문에 삽입)
      try {
  ...
  if (clipOk) {
    // 에디터 영역 클릭 후 붙여넣기
    console.log('   클립보드 복사 OK, 에디터에 붙여넣기...');
    ...
    if (check.ok) {
      ...
      console.log('   ⚠️ 본문 확인 실패: ' + (check.error || ''));
    }
  } else {
  ...
  console.log(saved ? '   ✅ 저장 버튼 클릭됨' : '   ⚠️ 저장 버튼 못 찾음');

  await sleep(5000);
  
  console.log('\n✅ 블로그 포스팅 완료!');
  console.log('   제목: ' + title);
  console.log('   이미지: 2장 업로드');
  console.log('   SEO: 100% 적용');
  console.log('   정렬: 센터 정렬');
  ...
  var finalCheck = await page.evaluate(function() {
    try {
      var ed = SmartEditor._editors['blogpc001'];
      return {
        title: ed.getTitle ? ed.getTitle() : '',
        contentLen: ed.getContentText ? ed.getContentText().length : 0
      };
    } catch(e) { return { error: e.message }; }
  });
  console.log('\n📋 최종 확인:', JSON.stringify(finalCheck));
