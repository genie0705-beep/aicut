...
(async () => {
  ...
  if (fi) {
    ...
    for (var s = 0; s < 3; s++) {
      var nxt = await igPage.evaluate(function() {
        ...
        for (var i = 0; i < btns.length; i++) {
          ...
          if ((t === '다음' || t === 'Next') && !btns[i].disabled) { btns[i].click(); return true; }
        }
        return false;
      });
    ...
    var shr = await igPage.evaluate(function() {
      ...
      for (var i = 0; i < btns.length; i++) {
        var t = (btns[i].innerText || '').trim();
        if ((t === '공유하기' || t === 'Share') && !btns[i].disabled) { btns[i].click(); return true; }
      }
      return false;
    });
