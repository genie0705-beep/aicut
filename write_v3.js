...
async function main() {
  ...
  await f.evaluate(() => {
    ...
    if (canvas) {
      canvas.focus();
      // Click on the components wrap to get focus
  ...
  await f.evaluate(() => document.execCommand('copy'));
  console.log('✅ 2/4 클립보드 복사');
  await f.waitForTimeout(300);

  // 3. 다시 canvas focus 후 paste
  await f.evaluate(() => {
    // Focus First Text (SE4 전용)
    try {
    ...
    // Also click selection
    const sel = document.querySelector('.se-selection');
    if (sel) { sel.click(); sel.focus(); }
  });
  await f.waitForTimeout(500);
  
  await page.keyboard.press('Control+v');
  console.log('✅ 3/4 본문 붙여넣기');
  ...
  const check = await f.evaluate(() => {
    ...
    const result = { docKeys: Object.keys(d) };
    ...
    if (d.blocks && Array.isArray(d.blocks)) {
      ...
      result.chars = chars;
      if (b.length > 0) {
        result.first = { type: b[0].type, text: (b[0].text||'').substring(0,50) };
      }
    } else {
      result.blocks = 'missing or not array';
    }
    
    if (d.components) {
      result.components = d.components.length;
      result.imgComps = d.components.filter(c => c.fileName).length;
    }
    
    const canvas = document.querySelector('.se-canvas');
    result.canvasTextLen = canvas ? (canvas.innerText || '').length : 0;
    result.canvasText = canvas ? (canvas.innerText || '').substring(0, 100) : 'no canvas';
    
    return result;
  });
  
  console.log('\n📊 붙여넣기 결과:', JSON.stringify(check, null, 2));

  if (!check.blocks || check.chars < 500) {
    console.log('⚠️ 붙여넣기 실패. setDocumentData로 전환');
    
    // fallback: setDocumentData
    const fallbackBlocks = clipHTML.split('\n').filter(l => l.trim()).map(line => {
  ...
  const final = await f.evaluate(() => {
    ...
    if (d.blocks && Array.isArray(d.blocks)) {
      ...
      let chars = 0;
      b.forEach(bl => { if (bl.text) chars += bl.text.length; });
      result.dataModel = { total: b.length, types: counts, chars };
    }
    
    result.images = d.components ? d.components.filter(c => c.fileName).length : 0;
    result.canvasTextLen = canvas ? (canvas.innerText || '').length : 0;
    result.canvasText = canvas ? (canvas.innerText || '').substring(0, 150) : 'no canvas';
