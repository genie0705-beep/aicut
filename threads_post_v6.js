...
(async () => {
  ...
  for (let i = 0; i < POSTS.length; i++) {
    ...
    try {
      // 홈으로 이동
      await page.goto('https://www.threads.com/', { waitUntil: 'load', timeout: 20000 });
      await sleep(3000);
      ...
      if (!clicked) {
        console.log('  새 스레드 버튼 없음, 직접 클릭 시도');
        // SVG 아이콘 방식 (펜 아이콘 등)
        const penBtn = await page.$('[aria-label*="새"], [aria-label*="작성"], [aria-label*="New"]');
        if (penBtn) await penBtn.click();
        else { console.log('  ❌ 버튼 못 찾음'); continue; }
      }
      await sleep(2000);
      ...
      console.log(`  게시버튼: ${JSON.stringify(btnState)}`);
    ...
    console.log(`  다음 게시까지 ${wait/1000}초 대기...`);
    await sleep(wait);
  }
