// 보험사 IG 캐러셀 — multiple 속성 추가 후 업로드
...
async function main() {
  ...
  // file input에 multiple 속성 추가 후 파일 설정
  const inputs = await p.$$('input[type="file"]');
  if (inputs.length) {
    // JavaScript로 multiple 속성 추가
    await p.evaluate(() => {
    ...
    try {
      ...
      console.log('   여러 파일 실패:', e.message);
      // 하나씩 업로드 — file chooser 이벤트 활용
      console.log('   → 순차 업로드 시도');
      for (let i = 0; i < FILES.length; i++) {
        await inputs[0].setInputFiles([FILES[i]]);
        await p.waitForTimeout(2000);
        console.log(`   ${i+1}/${FILES.length} 업로드`);
      }
    }
  }
