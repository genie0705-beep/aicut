...
async function main() {
  ...
  const breakpoints = [10, 27, 43, 59, 84]; // 각 섹션 마지막 라인 인덱스
  const imgFiles = [
  ...
  let startIdx = 0;
  for (let si = 0; si < breakpoints.length; si++) {
    const endIdx = breakpoints[si];
    const imgFile = imgFiles[si];
    ...
    try {
      ...
      console.log('  ⚠️ 이미지 업로드 실패: ' + imgFile);
    }

    // 텍스트
    const textLines = lines.slice(startIdx, endIdx + 1);
    await wp.evaluate((text) => {
      SmartEditor._editors['blogpc001']._editingService.writeTextWithSoftLineBreak(text);
    }, textLines.join('\n'));
    await wp.waitForTimeout(300);

    console.log('  [' + (si+1) + '/5] ' + imgFile + ' + 텍스트');
    startIdx = endIdx + 1;
  }
  ...
  const v = await wp.evaluate(() => {
    ...
    return {
      ...
      이미지정렬: Array.from(document.querySelectorAll('.se-section-image')).filter(s => (s.style.margin||'').includes('auto')).length + '/5',
