...
// =====================================================
// 전체 본문 (writeTextWithSoftLineBreak 1회 호출)
// 빈 줄 = 문단 구분, 빈 줄 2개 연속 = 섹션 구분
// =====================================================
const CONTENT = [
  // === 섹션 1: 도입부 (main 이미지 뒤) ===
  '보험설계사 FP라면 상상해보세요.',
...
async function main() {
  ...
  if (!seReady) { console.log('SE4 에디터를 불러올 수 없습니다.'); await b.close(); return; }

  console.log('✅ 1. 에디터 로딩 완료');
  ...
  // 본문 전체 입력 (writeTextWithSoftLineBreak 1회)
  ...
  for (let i = 0; i < imgFiles.length; i++) {
    ...
    try {
      ...
      console.log(`  🖼️ 이미지 ${i+1}/5 업로드 완료`);
    } catch (e) {
  ...
  const v = await wp.evaluate(() => {
    ...
    return {
      ...
      머지없음: !ft.includes('알려드립니다.☀') && !ft.includes('되었습니다.📋') && !ft.includes('됩니다.🎯'),
      해시태그: hashCount + '개',
