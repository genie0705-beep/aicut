...
const ALT_MAP = {};
SECTIONS.forEach(s => {
  const name = s.img.replace('.png', '');
  if (name.includes('main')) ALT_MAP[s.img] = '보험마케팅 FP 숏폼영상 편집 아웃소싱 에이컷';
  else if (name.includes('card1')) ALT_MAP[s.img] = '상반기 보험 마케팅 트렌드 영상편집외주 숏폼마케팅';
  else if (name.includes('card2')) ALT_MAP[s.img] = '하반기 FP 마케팅 숏폼 영상 전략 영상편집';
  else if (name.includes('card3')) ALT_MAP[s.img] = '보험설계사 영상 마케팅 예약률 상승 사례 에이컷';
  else if (name.includes('cta')) ALT_MAP[s.img] = '보험 마케팅 아웃소싱 에이컷 무료상담 숏폼';
});
...
async function writeText(wp, lines) {
...
async function main() {
  ...
  for (let i = 0; i < SECTIONS.length; i++) {
    ...
    // 1) 이미지 먼저 업로드 (커서가 현재 위치에 이미지 삽입)
    try {
      await uploadImage(wp, fullPath);
      console.log(`  🖼️ 이미지 업로드 완료`);
    } catch (e) {
      console.log(`  ⚠️ 이미지 업로드 실패: ${e.message}`);
    }

    // 2) 텍스트 입력
    await writeText(wp, sec.text);
    console.log(`  📝 텍스트 입력 완료 (${sec.text.filter(l => l !== '').length}문단)`);
  }
  ...
  const v = await wp.evaluate(() => {
    ...
    const imgsArr = Array.from(imgs).map(img => ({
      size: img.naturalWidth + 'x' + img.naturalHeight,
      alt: img.getAttribute('alt') || '(없음)',
      responsive: img.style.width === '100%' ? '반응형' : (img.naturalWidth === 700 ? '대표' : '고정')
    }));
    ...
    return {
      본문: ft.length + '자',
      이미지: imgs.length + '장',
      '이미지_배치': imgsArr.map((img, i) => `[${i}] ${img.size} alt="${img.alt}" ${img.responsive}`),
