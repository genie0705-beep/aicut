...
(async () => {
  ...
  const hasSE = await frame.evaluate(() => typeof SmartEditor !== 'undefined');
  console.log('SE4:', hasSE ? '✅' : '❌');
  
  if (!hasSE) { await b.close(); return; }
  ...
  const fullText = [
    ...
    '',
    '여행숏폼 제주도여행 숏폼촬영 제주항공권 여행영상편집 제주도호텔 제주맛집 제주가볼만한곳 후쿠오카여행 숏폼마케팅 영상편집외주 에이컷 여름휴가 제주도펜션 릴스촬영 쇼츠촬영 여행크리에이터 스마트폰촬영 제주도항공권 후쿠오카항공권 30초숏폼 여행브이로그 숏폼편집 여행릴스 제주공항 여행꿀팁 촬영꿀팁 영상편집 쇼츠편집 릴스편집'
  ].join('\n');
