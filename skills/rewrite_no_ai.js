...
async function main() {
  ...
  // 섹션별 이미지 + 텍스트
  const sections = [
    { textStart: 0, textEnd: 10, imgIdx: 0 },
    { textStart: 12, textEnd: 27, imgIdx: 1 },
    { textStart: 29, textEnd: 44, imgIdx: 2 },
    { textStart: 46, textEnd: 61, imgIdx: 3 },
    { textStart: 63, textEnd: 87, imgIdx: 4 }
  ];
  ...
  for (let si = 0; si < sections.length; si++) {
    ...
    const imgFile = IMG_FILES[sec.imgIdx];
    const fullPath = path.join(WS, imgFile);
    ...
    // 텍스트 입력
    const textLines = lines.slice(sec.textStart, sec.textEnd + 1);
    await wp.evaluate((text) => {
      SmartEditor._editors['blogpc001']._editingService.writeTextWithSoftLineBreak(text);
    }, textLines.join('\n'));
    await wp.waitForTimeout(300);

    console.log(`  섹션 ${si+1}/5: 이미지 + 텍스트 완료`);
  }

  console.log('2. 본문+이미지 입력 완료');
  ...
  const v = await wp.evaluate(() => {
    ...
    return {
      본문: ft.length + '자',
      이미지: imgs.length + '장',
      이미지정렬: Array.from(sections).filter(s => (s.style.margin||'').includes('auto')).length + '/' + sections.length,
      모듈정렬: Array.from(modules).filter(m => m.style.textAlign === 'center').length + '/' + modules.length,
      ...
      AICUT_제거됨: !ft.includes('AICUT') // 본문에 AICUT 텍스트가 없어야 함 (이미지 CTA에서 제거)
    };
  });
